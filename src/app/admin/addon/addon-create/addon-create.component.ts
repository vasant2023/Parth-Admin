...
@Component({
  ...
  styleUrls: ['./addon-create.component.css']
})
export class AddonCreateComponent implements OnInit {
  ...
  submitCategory() {
    if (this.isLoading == false) {
      ...
      formData.append("title", this.categoryObj.category);
      formData.append("sub_title", this.categoryObj.sub_title);
      formData.append("short_description", this.categoryObj.category);
      ...
      if (this.category_ID) {
        ...
        this.adminService
          ...
          .subscribe((response: { success: number; message: string }) => {
            if (response.success == 1) {
              this.router.navigate(["admin/item-addon"]);
            }

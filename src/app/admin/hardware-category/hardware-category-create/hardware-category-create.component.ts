...
@Component({
  selector: 'app-hardware-category-create',
  templateUrl: './hardware-category-create.component.html',
  styleUrls: ['./hardware-category-create.component.css']
})
export class HardwareCategoryCreateComponent implements OnInit {
  ...
  categoryDetails() {
    this.adminService.categoryDetails(this.category_ID).subscribe((response: { success: number, message: string, category: any }) => {
      if (response.success == 1) {
        ...
        this.categoryObj.parent_category = parseInt(response.category.parent_category);
      } else {
    ...
    if (this.isLoading == false) {
      ...
      if (this.category_ID) {
        ...
        this.adminService.updateCategory(formData).subscribe((response: { success: number, message: string }) => {
          if (response.success == 1) {
            this.router.navigate(['admin/hardware-category']);
          }
